...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1F2A44] to-[#2A3855]">
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-black text-white font-serif-title">Tableau de bord</h1>
          <p className="text-[#C6A75E]/60 font-mono text-sm mt-1">Gérez les demandes de devis</p>
        </div>

        {/* Stats Cards */}
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
          <div className="bg-[#1F2A44] border border-[#C6A75E]/20 rounded-xl p-4 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between">
              <FileText className="text-[#C6A75E] opacity-60 group-hover:opacity-100" size={20} />
          ...
          <div className="bg-[#1F2A44] border border-[#C6A75E]/20 rounded-xl p-4 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between">
              <Clock className="text-[#C6A75E] opacity-60 group-hover:opacity-100" size={20} />
          ...
          <div className="bg-[#1F2A44] border border-[#C6A75E]/20 rounded-xl p-4 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between">
              <Eye className="text-yellow-400 opacity-60 group-hover:opacity-100" size={20} />
          ...
          <div className="bg-[#1F2A44] border border-[#C6A75E]/20 rounded-xl p-4 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between">
              <CheckCircle className="text-green-400 opacity-60 group-hover:opacity-100" size={20} />
          ...
          <div className="bg-[#1F2A44] border border-[#C6A75E]/20 rounded-xl p-4 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between">
              <Archive className="text-gray-400 opacity-60 group-hover:opacity-100" size={20} />
          ...
          <div className="bg-[#1F2A44] border border-[#C6A75E]/20 rounded-xl p-4 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between">
              <TrendingUp className="text-[#C6A75E] opacity-60 group-hover:opacity-100" size={20} />
        ...
        {/* Filters */}
        <div className="bg-[#1F2A44] rounded-xl border border-[#C6A75E]/20 p-4 mb-8">
          <div className="flex flex-wrap gap-4 items-center">
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-[#2A3855] border border-[#C6A75E]/20 rounded-lg text-white placeholder-white/40 focus:outline-none focus:border-[#C6A75E] transition-colors"
                />
              </div>
            </div>
            ...
            <select
              ...
              className="px-4 py-2 bg-[#2A3855] border border-[#C6A75E]/20 rounded-lg text-white focus:outline-none focus:border-[#C6A75E] transition-colors"
            >
            ...
            <button
              onClick={exportCSV}
              className="flex items-center gap-2 px-4 py-2 bg-[#C6A75E] text-[#1F2A44] rounded-lg font-semibold hover:bg-[#B8963A] transition-all"
            >
              <Download size={16} />
            ...
            <button
              onClick={() => fetchDevis()}
              className="flex items-center gap-2 px-4 py-2 border border-[#C6A75E]/40 text-[#C6A75E] rounded-lg hover:bg-[#C6A75E]/10 transition-all"
            >
        ...
        <div className="bg-[#1F2A44] rounded-xl border border-[#C6A75E]/20 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#2A3855] border-b border-[#C6A75E]/20">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-mono text-[#C6A75E] uppercase tracking-wider">Date</th>
                    <th className="px-6 py-4 text-left text-xs font-mono text-[#C6A75E] uppercase tracking-wider">Client / Établissement</th>
                    <th className="px-6 py-4 text-left text-xs font-mono text-[#C6A75E] uppercase tracking-wider">Détails</th>
                    <th className="px-6 py-4 text-left text-xs font-mono text-[#C6A75E] uppercase tracking-wider">Urgence</th>
                    <th className="px-6 py-4 text-left text-xs font-mono text-[#C6A75E] uppercase tracking-wider">Statut</th>
                    <th className="px-6 py-4 text-left text-xs font-mono text-[#C6A75E] uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#C6A75E]/10">
                  {filteredDevis.map((devisItem) => {
                    ...
                    return (
                      <tr key={devisItem.id} className="hover:bg-[#2A3855]/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-mono border ${getUrgenceColor(devisItem.urgence)}`}>
                            <Zap size={10} />
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            <select
                              ...
                              className="px-2 py-1 bg-[#2A3855] border border-[#C6A75E]/20 rounded-lg text-white text-sm focus:outline-none focus:border-[#C6A75E]"
                            >
                              <option value="nouveau">🆕 Nouveau</option>
                              <option value="traité">✓ Traité</option>
                              <option value="archivé">📦 Archivé</option>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex justify-between items-center px-6 py-4 border-t border-[#C6A75E]/20 bg-[#2A3855]">
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page === 1}
                className="flex items-center gap-1 px-3 py-1 bg-[#1F2A44] border border-[#C6A75E]/20 rounded-lg text-white/60 disabled:opacity-50 disabled:cursor-not-allowed hover:border-[#C6A75E] transition-all"
              >
                <ChevronLeft size={14} />
      ...
      {isModalOpen && selectedDevis && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1F2A44] rounded-xl max-w-2xl w-full max-h-[80vh] overflow-auto border border-[#C6A75E]/20">
            <div className="sticky top-0 bg-[#1F2A44] border-b border-[#C6A75E]/20 p-4 flex justify-between items-center">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-white/40 text-xs font-mono mb-1">Nom</label>
                  <p className="text-white">{selectedDevis.nom}</p>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-white/40 text-xs font-mono mb-1">Téléphone</label>
                  <p className="text-white">{selectedDevis.telephone}</p>
                </div>
                <div>
                  <label className="block text-white/40 text-xs font-mono mb-1">Établissement</label>
                  <p className="text-white">{selectedDevis.etablissement} - {selectedDevis.surface} m²</p>
                </div>
                <div>
                  <label className="block text-white/40 text-xs font-mono mb-1">Nuisibles</label>
                  <p className="text-white">{selectedDevis.nuisibles.join(', ')}</p>
                </div>
                <div>
                  <label className="block text-white/40 text-xs font-mono mb-1">Urgence</label>
                  <p className="text-white">{selectedDevis.urgence}</p>
                </div>
              </div>
              {selectedDevis.message && (
                <div>
                  <label className="block text-white/40 text-xs font-mono mb-1">Message</label>
                  <p className="text-white bg-[#2A3855] p-3 rounded-lg">{selectedDevis.message}</p>
                </div>
              )}
              <div className="pt-4">
                <label className="block text-white/40 text-xs font-mono mb-1">Date de création</label>
